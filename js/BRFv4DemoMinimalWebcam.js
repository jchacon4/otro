...
/////AQUIII
    ...
    if(face.state === brfv4.BRFState.FACE_TRACKING_START ||
      ...
      $(".animated-bg").css({"background-image" : "url('lupits.gif')"});
      var audio = document.getElementsByTagName("audio")[0];
      audio.play();
      var XX = face["translationX"];
      var YY = face["translationY"];
      var timer;

      var circle = $("#circle");
      circle.addClass('moving');
      circle.css({left: XX +'px', top: YY +'px'});  //
      ...
      var audio = document.getElementsByTagName("audio")[0];
      $(".animated-bg").css({"background-image" : "url('')"});
      audio.pause();
      audio.currentTime = 0;
      var circle = $("#circle");
      circle.removeClass('moving');
      circle.css({left: 0 +'px', top: 0 +'px'});  //
    }
  }
};
